...
import { Bell, Search, RefreshCw, X, Activity, Volume2, VolumeX } from 'lucide-react'
...
export default function Header({ onRefresh }) {
  ...
  const [isAudioEnabled, setIsAudioEnabled] = useState(soundEngine.enabled)
    ...
    <header className="header">
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 8 }}>
          ...
          <button
            onClick={toggleAudio}
            style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid var(--glass-border)', borderRadius: 10, padding: '8px 10px', cursor: 'pointer', color: isAudioEnabled ? 'var(--low)' : 'var(--text-muted)', display: 'flex', transition: 'all 0.2s' }}
